...
///
/// While it would be possible to expand these with additional bitwise operations such as [`BitAnd`](std::ops::BitAnd) and
/// [`Not`](std::ops::Not), the preferred way to manipulate flags is to use the `with` and `without` methods rather than using the bitwise
